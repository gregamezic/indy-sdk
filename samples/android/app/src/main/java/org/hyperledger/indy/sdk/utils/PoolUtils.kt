...
import org.hyperledger.indy.sdk.utils.EnvironmentUtils.getTmpPath
import org.hyperledger.indy.sdk.utils.PoolJson.poolJson
import java.io.File
...
object PoolUtils {
    ...
    private const val DEFAULT_POOL_NAME = "default_pool"
    ...
    private fun createGenesisTxnFile(filename: String, context: Context): File {
        ...
        // helper class that return json pool
        val defaultTxns = poolJson
        val file = File(path)
    ...
    fun createPoolLedgerConfig(context: Context): String {
        val genesisTxnFile = createGenesisTxnFile("temp.txn", context)
        Log.d(TAG, "createPoolLedgerConfig: " + genesisTxnFile.absolutePath)
        val createPoolLedgerConfigJSONParameter =
            CreatePoolLedgerConfigJSONParameter(genesisTxnFile.absolutePath)

        // create new pool ledger config
        Pool.createPoolLedgerConfig(DEFAULT_POOL_NAME, createPoolLedgerConfigJSONParameter.toJson())
            .get()
